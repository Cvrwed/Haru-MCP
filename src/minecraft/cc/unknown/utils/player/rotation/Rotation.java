...
public class Rotation implements Loona {
	 ...
	 *
	 * @param sensitivity The mouse sensitivity value (default:
	...
	public Rotation fixedSensitivity(float sensitivity) {
		...
		this.yaw = getFixedSensitivityAngle(this.yaw, RotationManager.serverRotation.getYaw(), gcd);
		this.pitch = MathHelper.clamp_float(
				getFixedSensitivityAngle(this.pitch, RotationManager.serverRotation.getPitch(), gcd), -90f, 90f);

		return this;
	}
	
	public void toPlayer(EntityPlayer player) {
		if (Double.isNaN(player.rotationYaw) || Double.isNaN(player.rotationPitch))
			return;
