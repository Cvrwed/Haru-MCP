...
@Info(name = "Trajectories", category = Category.Visuals)
	...
	public void onRender3D(final RenderEvent e) {
		if (e.is3D()) {
			...
			if (itemStack != null && (itemStack.getItem() instanceof ItemSnowball
				...
				final EntityLivingBase thrower = mc.player;
				...
				rotationPitch = (prevRotationPitch = (float) (MathHelper.func_181159_b(y, MathHelper.sqrt_double(x * x + z * z))
						* 180.0 / Math.PI));
				boolean b = true;
				int ticksInAir = 0;
				while (b) {
				...
				if (this.positions.size() > 1) {
					...
					for (final Vec3 vec5 : this.positions) {
						worldrenderer.func_181662_b((float) vec5.xCoord - mc.getRenderManager().renderPosX,
								(float) vec5.yCoord - mc.getRenderManager().renderPosY,
								(float) vec5.zCoord - mc.getRenderManager().renderPosZ).func_181675_d();
					}
					tessellator.draw();
					if (m != null) {
