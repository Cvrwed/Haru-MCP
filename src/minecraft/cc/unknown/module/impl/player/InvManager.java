...
public class InvManager extends Module {
		...
		if (e.isPre()) {
			...
			for (int i = 0; i < INVENTORY_SLOTS; ++i) {
				...
				if (item instanceof ItemArmor) {
					...
					if (armor.armorType == 0) {
						if (bestHelmet == null || damageReductionItem > getArmorDamageReduction(
								mc.player.inventory.getStackInSlot(bestHelmet))) {
							bestHelmet = i;
						}
					}
					...
					if (armor.armorType == 1) {
						if (bestChestPlate == null || damageReductionItem > getArmorDamageReduction(
								mc.player.inventory.getStackInSlot(bestChestPlate))) {
							bestChestPlate = i;
						}
					}
					...
					if (armor.armorType == 2) {
						if (bestLeggings == null || damageReductionItem > getArmorDamageReduction(
								mc.player.inventory.getStackInSlot(bestLeggings))) {
							bestLeggings = i;
						}
					}
					...
					if (armor.armorType == 3) {
						if (bestBoots == null || damageReductionItem > getArmorDamageReduction(
								mc.player.inventory.getStackInSlot(bestBoots))) {
							bestBoots = i;
						}
					}
				...
				if (item instanceof ItemSword) {
					...
					if (bestSword == null
							|| damage > getSwordDamage(mc.player.inventory.getStackInSlot(bestSword))) {
						bestSword = i;
					}
				}
				...
				if (item instanceof ItemPickaxe) {
					...
					if (bestPickaxe == null
							|| mineSpeed > getMineSpeed(mc.player.inventory.getStackInSlot(bestPickaxe))) {
						bestPickaxe = i;
					}
				}
				...
				if (item instanceof ItemAxe) {
					final float mineSpeed = getMineSpeed(itemStack);
					if (bestAxe == null || mineSpeed > getMineSpeed(mc.player.inventory.getStackInSlot(bestAxe))) {
						bestAxe = i;
					}
				}
				...
				if (item instanceof ItemBlock && ((ItemBlock) item).getBlock().isFullCube()) {
					...
					if (bestBlock == null
							|| amountOfBlocks > mc.player.inventory.getStackInSlot(bestBlock).stackSize) {
						bestBlock = i;
					}
				}
				...
				if (item instanceof ItemPotion) {
					...
					if (bestPotion == null && ItemPotion.isSplash(itemStack.getMetadata())
						...
						for (final PotionEffect potion : mc.player.getActivePotionEffects()) {
	...
	private void throwItem(final int slot) {
		try {
			if (!movedItem) {
				openInventory();
				playerController.windowClick(mc.player.inventoryContainer.windowId, slot, 1, 4, mc.player);
				movedItem = true;
			}
	...
	private void equipArmor(final int slot) {
		try {
			if (slot > 8 && !movedItem) {
				openInventory();
				playerController.windowClick(mc.player.inventoryContainer.windowId, slot, 0, 1, mc.player);
				movedItem = true;
			}
		} catch (final IndexOutOfBoundsException ignored) {
		}
