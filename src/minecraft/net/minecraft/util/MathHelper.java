...
public class MathHelper {
	...
	public static <T extends Number> T wrapAngle(T value, double wrapRange) {
	    if (value instanceof Float) {
	        float floatValue = ((Float) value) % (float) wrapRange;
	        if (floatValue >= wrapRange / 2.0F) {
	            floatValue -= (float) wrapRange;
	        }
	        if (floatValue < -wrapRange / 2.0F) {
	            floatValue += (float) wrapRange;
	        }
	        ...
	        double doubleValue = ((Double) value) % wrapRange;
	        if (doubleValue >= wrapRange / 2.0) {
	            doubleValue -= wrapRange;
	        }
	        if (doubleValue < -wrapRange / 2.0) {
	            doubleValue += wrapRange;
	        }
	 ...
	 * check
	 */
	public static Number wrapAngle180(Number value) {
	    if (value instanceof Float) {
	        float floatValue = ((Float) value) % 360.0f;
	        ...
	        double doubleValue = ((Double) value) % 360.0;
	        if (doubleValue >= 180.0) {
	            doubleValue -= 360.0;
	        }
	        if (doubleValue < -180.0) {
	            doubleValue += 360.0;
